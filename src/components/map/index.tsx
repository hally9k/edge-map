...
const TrackerMapComponent: React.FC<Props> = ({ google }: Props) => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      if (trackerName) {
        try {
          fetch(
            `https://lqltw7pnb3.execute-api.ap-southeast-2.amazonaws.com/alpha?tracker=${trackerName}`
          )
    ...
    <div className="App">
      <div className="wrapper">
        ...
        <button
          ...
          onClick={() => setTrackerName(inputValue)}
        >
          Track
        </button>
      </div>
      <Map
        onReady={(_, map) => setMap(map)}
        google={google}
        initialCenter={CENTER}
        zoom={13}
        disableDefaultUI={true}
      >
        {tracker && <Marker position={tracker} title={tracker.device}></Marker>}
      </Map>
    </div>
  )
}

export const TrackerMap = GoogleApiWrapper({
